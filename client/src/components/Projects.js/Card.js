...
const Card = (props) => {
  ...
  return (
    <div
      className="md:my-[3vh] my-[1vh] md:w-[31vw] md:h-[52vh] w-[70vw] h-[40vh] min-h-fit flex justify-center items-center"
      onClick={() => {
        window.location.href = `/projectView?id=${project._id}`;
      }}
    >
      <div
        className="flex flex-col justify-between md:w-[28vw] md:h-[48vh] h-[35vh] w-[65vw] min-h-fit
          ...
          <div className="flex gap-2">
            {project.creatorId && project.creatorId.length > 0 ? (
              project.creatorId.map((user, index) => {
                user.thumbnail =
                  user.thumbnail || "images/defaultThumbnail.jpeg";
                return (
                  <a
