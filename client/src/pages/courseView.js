...
function CourseView() {
    ...
    useEffect(() => {
        ...
        getCourseReview(encodedData).then((response) => {
          console.log(response)
            setCourse(response.data);
            console.log(response.data.enrolledStudents.includes(localStorage.getItem('user')))
            setEnrolled(response.data.enrolledStudents.includes(localStorage.getItem('user')))
        }).catch((error) => {
    ...
    <div className="w-screen flex flex-col justify-center items-center mb-[10vh]">
      ...
      <div className="flex flex-col gap-2 w-[80vw] mt-[20vh] pl-[2vw]">
        ...
        <div className='flex gap-2 items-center'>
          ...
          {enrolled ? (
            <Button variant="contained" style={{ backgroundColor: '#0016DA' }} onClick={handleEnroll}>
              No
            </Button>
          ) : (
            <Button variant="contained" style={{ backgroundColor: '#0016DA' }} onClick={handleEnroll}>
      ...
      <div className="flex flex-col gap-2 w-[80vw] mt-[10vh] pl-[2vw]">
        ...
        <div>
          <div>
            {(course && course.techStacks && course.techStacks.length!==0) ? course.techStacks.map((techStack, index) => (
      ...
      <div className="flex flex-col gap-2 w-[80vw] mt-[10vh] pl-[2vw]">
        ...
        <div>
          {(course && course.commentsId && course.commentsId.length!==0) ? course.commentsId.map((comment, index) => (
